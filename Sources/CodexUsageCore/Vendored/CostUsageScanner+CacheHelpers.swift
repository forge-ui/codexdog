...
extension CostUsageScanner {
    ...
    static func requestedWindowExpandsCache(range: CostUsageDayRange, cache: CostUsageCache) -> Bool {
        ...
        // Moving a rolling window forward only adds new date partitions. Existing cached files
        // remain valid; a full backfill is required only when the requested start moves earlier.
        return range.scanSinceKey < cachedSince
    }
