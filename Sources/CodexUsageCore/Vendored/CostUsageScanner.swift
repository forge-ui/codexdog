...
enum CostUsageScanner {
    ...
    static let codexInactiveCacheRetentionDays = 90
    ...
    final class CodexCanonicalProjectPathResolver {
        ...
        func canonicalProjectPath(for projectPath: String?) -> String? {
            ...
            // CodexDog only displays aggregate usage, so resolving Git worktree roots adds
            // process-launch latency without changing any token or cost totals.
    ...
    private static func makeCodexRefreshPlan(
        ...
        nowMs: Int64,
        options: Options,
        checkCancellation: CancellationCheck?) -> CodexRefreshPlan
    {
        ...
        checkCancellation: CancellationCheck?) throws -> CostUsageDailyReport
    {
        ...
        if plan.shouldRefresh {
            ...
            let recentlyModifiedSince: Date? = if cache.lastScanUnixMs > 0, !plan.rootsChanged {
                // Include a small overlap so a file created while the previous scan was running
                // cannot fall into the gap between directory enumeration and cache persistence.
                Date(timeIntervalSince1970: Double(cache.lastScanUnixMs) / 1000 - 120)
            } else {
                coldCacheLookbackStart
            }
            var seenPaths: Set<String> = []
            var files: [URL] = []
            func appendFileIfNew(_ fileURL: URL) {
                // Normalize symlink aliases such as /var and /private/var before deduplication.
                // Otherwise one physical rollout can be scanned twice and the inode safeguard
                // will discard the cached contribution on the second pass.
            ...
            for root in plan.roots {
                ...
                for fileURL in rootFiles.sorted(by: { $0.path < $1.path }) {
                    appendFileIfNew(fileURL)
                }

                // Session files are partitioned by their creation date. A task created outside
                // the 30-day window can still be resumed today, so discover recently modified
                // files on every refresh instead of only when the cache is empty.
                if let recentlyModifiedSince {
                    let recentlyModifiedFiles = Self.listCodexRecentlyModifiedFiles(
                        root: root,
                        scanSinceKey: range.scanSinceKey,
                        scanUntilKey: range.scanUntilKey,
                        modifiedSince: recentlyModifiedSince)
            ...
            let inactiveFileCutoffMs = nowMs
                - Int64(Self.codexInactiveCacheRetentionDays * 24 * 60 * 60 * 1000)
            for key in cache.files.keys where !filePathsInScan.contains(key) {
                ...
                let shouldDrop = shouldDropAllUnscannedFiles
                    ...
                    || old.mtimeUnixMs < inactiveFileCutoffMs
            ...
            let shouldMergeCachedPriorityMetadata = !options.forceRescan && !plan.pricingChanged && !plan
                .priorityMetadataChanged && !plan.needsTurnIDCacheMigration && !plan.needsProjectMetadataMigration
            // CodexDog exposes a fixed rolling 30-day report. Keep aggregate cache bounds fixed
            // to the current request so daily buckets do not grow forever across app uptime.
            let retainedSinceKey = range.scanSinceKey
            let retainedUntilKey = range.scanUntilKey
            ...
            if plan.hasPriorityMetadata {
                cache.codexPriorityTurnKeys = Self.mergePriorityTurnKeys(
                    existing: shouldMergeCachedPriorityMetadata ? cache.codexPriorityTurnKeys : nil,
                ...
                cache.codexPriorityTurnIDsByDay = Self.mergePriorityTurnIDsByDay(
                    existing: shouldMergeCachedPriorityMetadata ? cache.codexPriorityTurnIDsByDay : nil,
        ...
        checkCancellation: CancellationCheck?) -> CodexFileScanContext
    {
        CodexFileScanContext(
            ...
            forceFullScan: options.forceRescan || plan.windowExpanded
                || plan.pricingChanged || plan.priorityMetadataChanged || plan.needsProjectMetadataMigration,
