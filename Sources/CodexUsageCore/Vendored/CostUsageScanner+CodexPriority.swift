...
extension CostUsageScanner {
    ...
    #if canImport(SQLite3)
    private final class CodexSQLiteCancellationProbe: @unchecked Sendable {
        let check: CancellationCheck

        init(check: @escaping CancellationCheck) {
            self.check = check
        }
    ...
    private static let codexSQLiteProgressCallback: @convention(c) (UnsafeMutableRawPointer?) -> Int32 = {
        context in
        guard let context else { return 0 }
        return Unmanaged<CodexSQLiteCancellationProbe>.fromOpaque(context)
            .takeUnretainedValue()
            .shouldInterrupt()
    }

    private static func installCodexSQLiteCancellationProbe(
        _ checkCancellation: CancellationCheck?,
        on db: OpaquePointer?) -> CodexSQLiteCancellationProbe?
    {
        guard let checkCancellation else { return nil }
        let probe = CodexSQLiteCancellationProbe(check: checkCancellation)
        sqlite3_progress_handler(
            db,
            1_000,
            self.codexSQLiteProgressCallback,
    ...
    static func codexPriorityTurns(
        ...
        checkCancellation: CancellationCheck? = nil) -> [String: CodexPriorityTurnMetadata]
    {
        ...
        let cancellationProbe = self.installCodexSQLiteCancellationProbe(checkCancellation, on: db)
        defer {
            sqlite3_progress_handler(db, 0, nil, nil)
            sqlite3_close(db)
            withExtendedLifetime(cancellationProbe) {}
        }
    ...
    private static func boundedCodexPriorityTurns(
        ...
        sinceDayKey: String?,
        untilDayKey: String?,
        checkCancellation: CancellationCheck?) -> [String: CodexPriorityTurnMetadata]
    {
